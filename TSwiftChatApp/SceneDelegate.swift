...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    //MARK: - check first run
    func checkFirstRun(){
        isFirstRun = UserDefaults.standard.bool(forKey: KFIRSTRUN)
        if !isFirstRun! {
            let statusArr = Status.allCases.map { status in
                status.rawValue
            }
            UserDefaults.standard.set(statusArr, forKey: KSTATUSLIST)
